-- 1) Fix verify_employee_credentials to use extensions.crypt explicitly
...
BEGIN
  ...
  -- Verify password using pgcrypto via extensions schema
  IF v_password_hash != extensions.crypt(_password, v_password_hash) THEN
    RETURN QUERY SELECT false, NULL::UUID, NULL::TEXT, NULL::TEXT, false, ARRAY[]::app_role[];
    RETURN;
  END IF;
...
-- 2) Public wrappers to use pgcrypto safely via RPC
...
AS $$
  SELECT extensions.gen_salt(type::text);
$$;
...
SET search_path = public
AS $$
  SELECT extensions.crypt(password::text, salt::text);
$$;